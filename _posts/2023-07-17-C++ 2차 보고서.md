...
<head>
  ...
  .page__content p > strong {
    ...
    .toc li.has-submenu ul {
      display: none;
    }

    .toc li.has-submenu.show ul {
      display: block;
      margin-top: 10px;
  ...
  <script>
    document.addEventListener("DOMContentLoaded", function () {
      ...
      // 각 최상위 TOC 항목에 클릭 이벤트 리스너 추가
      for (let i = 0; i < tocItems.length; i++) {
        const tocItem = tocItems[i];
        const subMenu = tocItem.querySelector("ul");
        if (subMenu) {
          tocItem.classList.add("has-submenu");
          tocItem.addEventListener("click", function () {
            // 하위 메뉴가 있는 경우에만 하위 메뉴를 보여줍니다.
