...
다음은 2차원 배열의 모든 요소를 ​​합산하는 함수입니다.

```
int sum_array(int array[2][3]) {
  int sum = 0;
  for (int i = 0; i < 2; i++) {
    for (int j = 0; j < 3; j++) {
      sum += array[i][j];
...
이 함수는 2차원 배열의 주소를 인수로 받습니다.

이 주소를 사용하여 함수는 배열의 모든 요소에 접근할 수 있습니다.

다음은 함수와 2차원 배열을 포인터에 연관시키는 예입니다.

```
int main() {
  int array[2][3] = {{1, 2, 3}, {4, 5, 6}};
  int *ptr = array;
  int sum = sum_array(ptr);
  cout << sum; // 출력: 15
}
```

이 코드는 2차원 배열의 주소를 포인터 변수 `ptr`에 저장합니다. 

그런 다음 함수 `sum_array()`를 호출하고 포인터 변수 `ptr`을 인수로 전달합니다. 

함수는 포인터 변수 `ptr`을 사용하여 배열의 모든 요소에 접근하고 요소의 합을 반환합니다. 

반환된 값은 변수 `sum`에 저장되고 `cout` 함수를 사용하여 출력됩니다.
