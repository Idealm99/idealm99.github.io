...
auto p1 = f1 // 자동 형 변환으로 바로 위 코드와 동일하다
```
...
함수 템플릿(Function Template)은 C++에서 코드의 재사용을 위해 사용되는 기능입니다. 함수 템플릿은 하나의 템플릿 선언으로 여러 종류의 함수를 생성할 수 있게 해줍니다. 템플릿을 사용하여 작성된 함수는 자동으로 컴파일러에 의해 인스턴스화되며, 각각의 인스턴스는 특정한 타입에 대해 맞춤화된 함수를 생성합니다.
...
함수 템플릿의 오버로딩(Function Template Overloading)은 동일한 이름을 가진 여러 개의 함수 템플릿을 정의하는 것을 말합니다. 오버로딩된 함수 템플릿은 매개변수의 타입이나 개수가 다르지만, 동일한 작업을 수행하는 다양한 버전의 함수를 생성할 수 있습니다.


함수 템플릿 제한(Template Specialization)은 일반적인 템플릿 동작을 변경하거나 특정 타입에 대해 특수한 구현을 제공하기 위해 사용됩니다. 템플릿 제한은 특정 타입에 대한 특화된 동작을 지정하고, 해당 타입에 대해서만 템플릿 인스턴스를 생성합니다.


명시적 특수화(Explicit Specialization)는 특정 타입에 대한 템플릿 제한을 명시적으로 정의하는 것을 말합니다. 명시적 특수화는 일반적인 템플릿 동작을 오버라이딩하고 특정 타입에 대해 사용자 정의 구현을 제공하는 데 사용됩니다.
...
또한, `add` 함수 템플릿을 부동소수점형에 대해서만 특수화하여 다른 동작을 제공하였습니다. `add<float>` 명

시적 특수화는 부동소수점형의 두 수를 더한 후 1.0을 추가로 더해 반환합니다.

위의 코드는 템플릿을 사용하여 여러 종류의 함수를 생성하고 오버로딩하며,

특정 타입에 대해 특수한 구현을 제공하는 템플릿 제한과 명시적 특수화의 예시입니다.
