...
왜냐하면 포인터 `pt`은 `const int`를 가리키고 있는데, `age`는 `int`이기 때문입니다. 

`const int`는 변경할 수 없는 `int`이기 때문에 `age`를 변경할 수 없는 `const int`로 가리킬 수 없습니다.

이 문제를 해결하려면 `age`를 `const int`로 선언해야 합니다.
...
이제 `pt`은 `age`를 변경할 수 없는 `const int`로 가리킬 수 있습니다.
