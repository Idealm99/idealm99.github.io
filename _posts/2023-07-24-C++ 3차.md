...
| 정적 기억 존속 시간    | 함수 바깥에서 정의된 변수 또는 "static" 키워드로 정의된 변수는 프로그램 실행 동안 계속 유지됩니다. |
| 쓰레드 존속 시간      | 멀티코어 프로세서를 사용하여 연산 작업을 쓰레드 단위로 처리하여 해당 쓰레드의 수명과 관련됩니다. |
...
* 메모리 풀 사용: 위치 지정 new 연산자를 사용하여 특정 메모리 영역(메모리 풀)에서 객체를 동적으로 생성할 수 있습니다.
  메모리 풀은 미리 할당된 메모리 공간을 관리하여 객체를 동적으로 생성하는데 사용되며,
  객체의 생성과 소멸을 반복적으로 수행해야 하는 상황에서 성능 향상을 도모할 수 있습니다.

* 특정 주소에 객체 배치: 위치 지정 new 연산자를 사용하여 객체를 특정 메모리 주소에 직접 배치할 수 있습니다. 이는 임베디드 시스템이나 하드웨어 제어와 같이 메모리 주소를 직접 다루는 경우에 유용합니다.

* 객체 직렬화와 역직렬화: 위치 지정 new 연산자는 객체 직렬화(serialization)나 역직렬화(deserialization)과 관련하여 특정 메모리 영역에 객체를 만들어야 할 때 사용될 수 있습니다.
...
이름이 없는 이름 공간 안에 선언된 이름들의 잠재 사용 범위는 이름을 명명하지 않은 공간을 포함하고 있는 선언 영역의 끝까지이다.
