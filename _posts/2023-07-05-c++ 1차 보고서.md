...
while 루프는 for 루프와 비교하여 몇 가지 장점이 있습니다. while 루프는 for 루프보다 코드가 간결합니다. 또한, while 루프는 특정 조건을 만족하는 동안만 코드를 실행해야 하는 경우에 유용합니다.

while 루프는 C++에서 매우 유용한 도구입니다. while 루프를 사용하여 프로그램의 특정 부분을 반복적으로 실행하거나, 특정 시간 동안 코드를 실행할 수 있습니다.

### 5.2.1 시간 지연 루프

인터넷에 검색해본 결과 C++에서 시간 지연 푸르를 사용하는 대표적인 방법중 하나인 sleep()이 있었다. 

시간 지연 루프를 생성하는 방법은 여러 가지가 있습니다. 가장 일반적인 방법은 `Sleep()` 함수를 사용하는 것입니다. `Sleep()` 함수는 특정 시간 동안 실행을 중단합니다. 예를 들어, 다음 코드는 2초 동안 실행을 중단합니다.
...
C++에서 do-while 루프는 최소 한 번 코드를 실행한 후 조건을 확인하는 반복문입니다. do-while 루프는 while 루프와 유사하지만, 조건을 먼저 확인하는 것이 아니라 코드를 먼저 실행한 후 조건을 확인합니다. do-while 루프는 다음과 같이 작성됩니다.
...
위 코드는 코드를 한 번 실행하고, 조건이 만족되는 동안 코드를 반복적으로 실행합니다. 조건이 만족되지 않으면 do-while 루프는 종료됩니다.

do-while 루프는 while 루프보다 코드가 간결하고, 특정 조건을 만족하는 동안만 코드를 실행해야 하는 경우에 유용합니다.
...
위 코드는 변수에 사용자로부터 입력을 받습니다. 변수는 문자열, 정수, 실수 등 다양한 자료형을 사용할 수 있습니다.

cin을 이용한 입력은 루프와 함께 사용하여 사용자로부터 여러 개의 입력을 받을 수 있습니다. 예를 들어, 다음 코드는 사용자로부터 10개의 정수를 입력받아 배열에 저장합니다.
...
cin을 이용한 입력은 사용자로부터 입력을 받는 데 매우 유용한 함수입니다. cin을 이용한 입력을 사용하여 사용자로부터 다양한 자료형의 입력을 받을 수 있습니다.
...
위 코드는 변수에 표준 입력 스트림에서 데이터를 읽고 저장합니다. 변수는 문자열, 정수, 실수 등 다양한 자료형을 사용할 수 있습니다.

`cin`을 이용한 입력은 루프와 함께 사용하여 표준 입력 스트림에서 여러 개의 데이터를 읽을 수 있습니다. 예를 들어, 다음 코드는 표준 입력 스트림에서 10개의 정수를 읽고 배열에 저장합니다.
...
두 함수는 모두 int형 값을 반환합니다. eof() 함수는 파일 끝에 도달하면 0을 반환하고, 파일 끝에 도달하지 않으면 -1을 반환합니다. fail() 함수는 파일 작업 중 오류가 발생하면 0을 반환하고, 파일 작업 중 오류가 발생하지 않으면 -1을 반환합니다.
...
중첩 루프는 한 루프 안에서 다른 루프를 반복하는 것입니다. 중첩 루프는 2차원 배열을 순회하거나, 2차원 그래프를 탐색하는 등 다양한 목적으로 사용할 수 있습니다.

2차원 배열은 한 줄에 여러 개의 요소를 저장하는 배열입니다. 2차원 배열은 행과 열로 구성되어 있습니다. 행은 가로로 늘어선 요소들의 집합이고, 열은 세로로 늘어선 요소들의 집합입니다.
